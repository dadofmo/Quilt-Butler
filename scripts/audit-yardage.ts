...
console.log("\n=== Album Cross: 48×60, 12\" block, no sashing ===");
{
  ...
  // accent squares cut 2.5", background triangle starts cut 2.875", and large outer triangle starts cut 4.875".
  const r = calculateYardage(s);
  const A = r.fabrics.find(f => f.fabric === "A")!;
  const B = r.fabrics.find(f => f.fabric === "B")!;
  const C = r.fabrics.find(f => f.fabric === "C")!;
  ...
  check("AlbumCross B triangle starts", B.pieces.find(p => /triangle/.test(p.label))!.count, 80);
  check("AlbumCross B triangle cut", B.pieces.find(p => /triangle/.test(p.label))!.w, 2.875);
  check("AlbumCross C triangle starts", C.pieces.find(p => /triangle/.test(p.label))!.count, 40);
  check("AlbumCross C triangle cut", C.pieces.find(p => /triangle/.test(p.label))!.w, 4.875);
...
console.log("\n=== Album Cross: 9\" block, odd block count, with sashing ===");
{
  ...
  // background triangle starts 2.375", large outer starts 3.875". Two vertical sashes, each 2.5" × 9.5".
  const r = calculateYardage(s);
  const A = r.fabrics.find(f => f.fabric === "A")!;
  const B = r.fabrics.find(f => f.fabric === "B")!;
  const C = r.fabrics.find(f => f.fabric === "C")!;
  ...
  check("AlbumCross(3) B triangle starts", B.pieces.find(p => /triangle/.test(p.label))!.count, 12);
  check("AlbumCross(3) B triangle cut", B.pieces.find(p => /triangle/.test(p.label))!.w, 2.375);
  check("AlbumCross(3) C triangle starts", C.pieces.find(p => /triangle/.test(p.label))!.count, 6);
  check("AlbumCross(3) C triangle cut", C.pieces.find(p => /triangle/.test(p.label))!.w, 3.875);
