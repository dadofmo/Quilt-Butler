...
export function calculateYardage(s: PlannerState): CalcResult {
  ...
  if (s.pattern === "simple-squares") {
    ...
    //     - 1 plain Fabric C square, u × u finished (outer corner)
    //     - 1 plain Fabric B square, u × u finished (inner corner)
    //     - 2 HST units (Fabric C + Fabric B). 1 C-start + 1 B-start square
    ...
    addSquares(reqs[r1Fab], "Round 1 triangle squares||then cut each square ONCE corner-to-corner on the diagonal", r1SqCount, round1Cut, s.fabricWidth);
    addSquares(reqs[r2Fab], "Round 2 corner triangle squares||then cut each square ONCE corner-to-corner on the diagonal", r2SqCount, round2Cut, s.fabricWidth);
