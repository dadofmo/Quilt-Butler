  ...
  return (
    <StepShell step={2} title="Quilt size & basics" subtitle="A few quick details so we can do the math." backTo="/">
      <div className="space-y-6">
                                                  ...
                                                  ? "Sashing separates each Four Patch block — common widths are 1.5\", 2\", 2.5\", or 3\". Use 0 for no sashing."
                                                    : isStreak
                                                      ...
                                                      : isShoofly
                                                        ...
                                                        : `Sashing separates each ${getPattern(pattern)?.name ?? "quilt"} block — common widths are 1.5", 2", 2.5", or 3". Use 0 for no sashing.`}
            </p>
            {!sashingValid && (
              <p className="text-destructive mt-2 text-sm font-medium">
        ...
        <button
          onClick={next}
          disabled={!blockSizeValid || !borderValid || (!isJellyRoll && !isFatQuarter && !fabricWidthValid) || (isJellyRoll && !stripCountValid) || (isFatQuarter && (!fqWidthValid || !fqHeightValid || !fqTrimValid || !fqCountValid)) || (isSashed && !sashingValid) || (isSnowball && !cornerAccentValid)}
